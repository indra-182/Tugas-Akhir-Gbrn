...
public class BaristaPanel extends JPanel {
    ...
    private void buatTampilan() {
        add(TampilanUtil.buatJudul("DATA BARISTA"), BorderLayout.NORTH);
    ...
    private void pasangEvent() {
        ...
        tabel.getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting() && tabel.getSelectedRow() >= 0) {
                ...
                Barista barista = tableModel.ambilBarista(baris);
    ...
    private void saringData() {
        ...
        if (kataKunci.isEmpty()) {
            ...
            penyaringTabel.setRowFilter(new RowFilter<BaristaTableModel, Integer>() {
                @Override
                public boolean include(Entry<? extends BaristaTableModel, ? extends Integer> entry) {
                    String kodeBarista = String.valueOf(entry.getValue(1)).toLowerCase();
                    String nama = String.valueOf(entry.getValue(2)).toLowerCase();
                    return kodeBarista.matches(pola) || nama.matches(pola);
                }
            });
        }
    }
    ...
    private void muatData() {
        try {
            tableModel.setData(baristaDao.ambilSemua());
    ...
    private void hapus() {
        ...
        if (!DialogUtil.confirm(this, "Hapus data barista terpilih?")) {
            return;
        }
        try {
    ...
    private static class BaristaTableModel extends AbstractTableModel {
        ...
        public Barista ambilBarista(int baris) {
